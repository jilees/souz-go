...
- [x] **Найденный и исправленный баг (при первом реальном деплое на SberBoom)**: `selection.Select` и `validation.ValidateWithLLM` не задавали `Model` в `providers.ChatRequest`, поэтому оба вызова уходили не на модель из `config.yaml`, а на захардкоженный `openai_compat.defaultModel = "gpt-5-nano"`. Для reasoning-моделей это фатально для валидатора: `gpt-5-nano` стабильно сжирал весь `maxTokens=1024` на скрытый reasoning (`finish_reason: length`, `content: null`), ответ никогда не долетал до JSON-вердикта, и `ValidateWithLLM`'s fail-closed поведение рейджектило **любой** скилл независимо от содержимого — воспроизведено вручную прямыми запросами к OpenRouter (с явным `model` вердикт парсился 6/6, без него — 0/4). Пофикшено: `nodes.SkillsConfig` получил поле `Model` (= `cfg.Model`), проброшено в оба вызова (`selection.Select(..., model, ...)`, `validation.Validate(..., model)` → `ValidateWithLLM(..., model)`); тесты на оба пакета проверяют, что `ChatRequest.Model` реально доходит до провайдера
- [x] **Второй найденный пробел (тот же деплой)**: `RunSkillCommand`'s `runtime: "bash"` жёстко искал бинарь `bash` — на SberBoom (BusyBox, только `sh`, без `bash`) это гарантированно падало на `"bash" is not available on this system"` для любого bash-скилла. Пофикшено симметрично уже существующему `pythonBinary()` (`python3` → `python`): новая `bashBinary()` в `pkg/tools/skills/exec.go` пробует `exec.LookPath("bash")`, при отсутствии — падает на `sh`. Явно не эмулирует bash-синтаксис (массивы, `[[ ]]`, process substitution останутся недоступны под `sh`) — только не даёт runtime упасть на старте там, где сам скрипт и так POSIX-совместим. Тест эмулирует отсутствие `bash` через `PATH`, ограниченный директорией с одним `sh` (`t.Setenv("PATH", ...)`), не полагаясь на реальное окружение CI/dev-машины
