...
Добавлен по запросу после Фазы 5. Третий `providers.LLMProvider`, `pkg/providers/codex/` — авторизация через OAuth Device Authorization Flow подписки ChatGPT (не статичный API-ключ, в отличие от `anthropic`/`openai_compat`), запросы к OpenAI Responses API (другой протокол, не Chat Completions).

- **Портирован напрямую в Go, не оставлен JVM-сайдкаром.** Реальный OAuth-флоу в Kotlin-оригинале оказался проще ожидаемого: Device Code Grant (RFC 8628) с ручным вводом кода, без браузерного redirect и локального callback-сервера (в отличие от MCP OAuth в том же Kotlin-проекте, который именно так и устроен). Код самодостаточен — зависит только от 4 полей токена, никаких desktop-API — поэтому JVM-сайдкар вернул бы то самое ограничение (JVM не помещается в 256MB), ради обхода которого затевался весь Go-порт.
- Токены (access/refresh/accountId/expiresAt) хранятся **не** в `config.yaml`, а в отдельном файле `{DataDir}/codex-token.json` — в отличие от статичного API-ключа, этот файл переписывается самим агентом при каждом рефреше (~раз в час), а `config.yaml` осознанно не переписывается программно нигде в проекте.
- Рефреш ленивый, перед каждым запросом (буфер 300с до истечения, без фонового таймера), с fallback на текущий токен при сетевом сбое рефреша.
- Codex не отдаёт по-токенные дельты текста даже в "стриминге" API — только целые items и финальный `response.completed`. `onChunk` в `ChatStream` никогда не вызывается — это точное соответствие поведению оригинала, не Go-упрощение.
- `cmd/souz-agent -codex-login` — интерактивная привязка аккаунта в терминале (печатает код и ссылку, поллит, сохраняет токен), отдельно от обычного запуска агента.
- Тесты полностью через `httptest` (все URL-эндпоинты — `var`, не `const`, специально для подмены), реальный OpenAI OAuth не используется нигде в тестах.
